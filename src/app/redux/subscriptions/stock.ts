...
export const stockApi = createApi({
	...
	baseQuery: fetchBaseQuery({
		baseUrl: process.env.NEXT_PUBLIC_STOCK_BASEURL,
	}),
	...
	endpoints: (builder) => ({
		fetchStockListings: builder.query<any, any>({
			query() {
				return `?function=TOP_GAINERS_LOSERS&apikey=${process.env.NEXT_PUBLIC_STOCK_APIKEY}`;
			},
		}),
		searchStockListings: builder.query<SearchStockListingResults, string>({
			query(searchTerm) {
				return `?function=SYMBOL_SEARCH&keywords=${searchTerm}&apikey=${process.env.NEXT_PUBLIC_STOCK_APIKEY}`;
			},
		}),
		getStockDetails: builder.query<StockDetails, string>({
			query(symbol) {
				return `?function=TIME_SERIES_INTRADAY&symbol=${symbol}&interval=5min&apikey=${process.env.NEXT_PUBLIC_STOCK_APIKEY}`;
			},
		}),
		getStockBySymbol: builder.query<Stock | null, string>({
			query: (symbol) =>
				`?function=GLOBAL_QUOTE&symbol=${symbol}&apikey=${process.env.NEXT_PUBLIC_STOCK_APIKEY}`,
